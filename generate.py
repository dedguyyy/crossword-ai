...
class CrosswordCreator():
    ...
    def ac3(self, arcs=None):
        ...
        for word in self.domains:
            for word_2 in self.domains:
                if word!=word_2 and self.crossword.overlaps[word, word_2] is not None:
                    arcs.append((word,word_2))
        while len(arcs)>0:
            x,y = arcs.pop(0)
            if self.revise(x,y):
                ...
                for z in self.domains:
                    if z!=x and z!=y and self.crossword.overlaps[z, x] is not None:
                        arcs.append((z,x))
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        for word in self.domains[var]:
            ...
            for var_2 in self.crossword.variables:
                if var_2!=var and self.crossword.overlaps[var, var_2] is not None and var_2 not in assignment:
                    for word_2 in self.domains[var_2]:
                        if word[self.crossword.overlaps[var, var_2][0]] != word_2[self.crossword.overlaps[var, var_2][1]]:
                            cnt+=1
            ans.append((cnt,word))
